...
func (_c *PollCreate) SetIsOpen(v bool) *PollCreate {
	_c.mutation.SetIsOpen(v)
	return _c
}

// SetNillableIsOpen sets the "is_open" field if the given value is not nil.
func (_c *PollCreate) SetNillableIsOpen(v *bool) *PollCreate {
	if v != nil {
		_c.SetIsOpen(*v)
	}
	return _c
...
func (_c *PollCreate) check() error {
	...
	if _, ok := _c.mutation.IsOpen(); !ok {
		return &ValidationError{Name: "is_open", err: errors.New(`ent: missing required field "Poll.is_open"`)}
	}
	if _, ok := _c.mutation.CreatedAt(); !ok {
